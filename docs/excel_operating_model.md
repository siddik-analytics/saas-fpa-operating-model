...
over the Phase 3–8 analytical stack. Built reproducibly from the committed marts by
`src/build_excel_model.py`; validated by `src/validate_excel_model.py` and
...
workbook, and then runs the full validation suite. `--skip-validation` builds without checking;
`--marts` and `--out` override the input and output paths.

The workbook is also regenerated by the normal project build:
...
To refresh the workbook alone against the committed marts without regenerating anything
upstream, run `python -m src.build_excel_model`. To refresh the marts first,
...
12. **PHASE1_SPEC section 10 names a five-tab workbook at `models/Helio_FPA_Operating_Model.xlsx`.**
    This phase builds eleven visible tabs at `excel/Helio_SaaS_FP&A_Operating_Model.xlsx`,
